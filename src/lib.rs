...
impl Config {
    ...
    pub fn build(&mut self) -> PathBuf {
        ...
        if target.contains("windows-gnu") {
            ...
            let ios_target = IosTarget::ios_target(&target)
            ...
            if !self.defined("CMAKE_OSX_ARCHITECTURES") {
                cmd.arg(format!("-DCMAKE_OSX_ARCHITECTURES={}", ios_target.cmake_target_arch()));
            }
...
impl IosTarget {
    ...
    fn ios_target(target_name: &str) -> Option<IosTarget> {
        if IosTarget::is_ios_target(target_name) {
            let rust_target_arch = target_name.split('-').nth(0).expect("Unknown architecture for iOS target.").into();
            Some(IosTarget {
                rust_target_arch,
            })
        } else {
